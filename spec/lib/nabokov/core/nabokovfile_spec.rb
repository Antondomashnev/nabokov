...
describe Nabokov::Nabokovfile do
  it "raises an exception if there is no nabokovfile at the given path" do
    expect { Nabokov::Nabokovfile.new('./nabokovfile.yml', testing_ui) }.to raise_error("Couldn't find nabokov file at './nabokovfile.yml'")
  end

  it "raises an exception if the file does not have a legit YAML syntax" do
    expect { Nabokov::Nabokovfile.new('spec/fixtures/nabokovfile_example_invalid.yaml', testing_ui) }.to raise_error("File at 'spec/fixtures/nabokovfile_example_invalid.yaml' doesn't have a legit YAML syntax")
  end

  it "assigns the localization_git_repo value from valid nabokovfile" do
    nabokovfile = Nabokov::Nabokovfile.new('spec/fixtures/nabokovfile_example.yaml', testing_ui)
