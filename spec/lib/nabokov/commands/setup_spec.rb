...
describe Nabokov::Setup do
  ...
  describe "run" do
    before do
      ...
      system('mkdir spec/fixtures/test_git_setup/git_folder/')
      system('git init spec/fixtures/test_git_setup/git_folder/')
    end
    ...
    context "when current git repo is not listed in pre_commit_file" do
      ...
      it "puts into pre commit line to tell that file is executable" do
        result = File.foreach("spec/fixtures/test_git_setup/existed_pre_commit_file").any? { |line|
